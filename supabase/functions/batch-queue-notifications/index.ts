...
serve(async (req) => {
  ...
  try {
    ...
    const {
      recipients, // Array of { user_id, recipient_email, recipient_phone }
      template_name,
      variables_template, // Common variables for all
      priority = 5
    } = await req.json()
    ...
    if (recipients.length > 1000) {
      return new Response(
        JSON.stringify({ error: 'Maximum 1000 recipients per batch' }),
        { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
      )
    }
    ...
    const queueItems = recipients.map(recipient => ({
      ...
      template_name,
      variables: { ...variables_template, ...recipient.variables },
