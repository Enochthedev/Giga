...
export interface IQueueManager {
  ...
  enqueueScheduledNotification(notification: ScheduledNotification): Promise<string>;
  enqueueBulkNotifications(notifications: QueuedNotification[]): Promise<string[]>;
  ...
  getQueueHealth(queueName?: string): Promise<QueueHealthCheck | QueueHealthCheck[]>;
  getJobProgress(jobId: string): Promise<JobProgress>;
  getJobsByStatus(queueName: string, status: JobStatus, limit?: number): Promise<QueuedNotification[]>;

  // Failed job management
  getFailedJobs(queueName: string, limit?: number, offset?: number): Promise<FailedJob[]>;
  retryFailedJob(jobId: string): Promise<boolean>;
  retryAllFailedJobs(queueName: string): Promise<{ retried: number; failed: number }>;
  ...
  updateQueueConfig(queueName: string, config: Partial<QueueConfiguration>): Promise<boolean>;
  ...
  // Bulk operations
  executeBulkOperation(operation: BulkQueueOperation): Promise<{ affected: number; errors: string[] }>;

  // Metrics and monitoring
  getQueueMetrics(queueName: string, timeRange?: { start: Date; end: Date }): Promise<QueueMetrics[]>;
  ...
  getScheduledJobs(limit?: number, offset?: number): Promise<ScheduledNotification[]>;
  updateScheduledJob(jobId: string, updates: Partial<ScheduledNotification>): Promise<boolean>;
